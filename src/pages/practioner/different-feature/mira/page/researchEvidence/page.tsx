...
export const ResearchEvidenceModal = ({ isOpen, onClose, title = "AI Investigator" }: ResearchEvidenceModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-110 bg-black/40 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full h-full max-w-[87.5rem] rounded-xl shadow-2xl flex overflow-hidden animate-in zoom-in-95 duration-200">
        
        {/* Left Sidebar: AI Investigator */}
        <div className="w-[23.75rem] bg-white border-r border-gray-200 flex flex-col shrink-0 h-full">
          <div className="p-6 flex-1 overflow-y-auto">
            ...
            <p className="text-[0.875rem] text-gray-700 italic leading-relaxed">
          ...
          <div>
            <h4 className="text-[0.6875rem] font-bold text-gray-400 uppercase tracking-wider mb-4">Analysis Logs</h4>
          ...
          <div className="border-t border-gray-200 p-6 bg-[#FAFAF9] flex flex-col gap-4 shrink-0">
             ...
             <div className="relative">
               <input 
                 ...
                 className="w-full bg-white border border-gray-200 text-[0.8125rem] rounded-xl pl-4 pr-10 py-3 outline-none focus:border-[#005EB8] transition-colors shadow-sm" 
               />
             ...
             <div className="flex items-center justify-between mt-2">
                <div className="flex items-center gap-3">
                   <button className="w-11 h-11 bg-red-50 hover:bg-red-100 rounded-full flex items-center justify-center transition-colors border border-red-100 shadow-sm" title="End Call">
                     <PhoneOff className="w-[1.125rem] h-[1.125rem] text-red-600" />
                   </button>
                   <button className="w-11 h-11 bg-white border border-gray-200 hover:bg-gray-100 rounded-full flex items-center justify-center transition-colors shadow-sm" title="Mute AI">
                     <MicOff className="w-[1.125rem] h-[1.125rem] text-gray-600" />
                   </button>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 bg-green-50 rounded-full border border-green-100">
                  <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_0.5rem_rgba(34,197,94,0.6)]"></span> 
                  <span className="text-[0.6875rem] font-bold text-green-700 uppercase tracking-wider">Active</span>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col bg-[#F8FAFC]">
          ...
          <div className="bg-white px-8 py-5 flex items-start justify-between shrink-0">
             <div>
                <span className="text-[#005EB8] text-[0.625rem] font-bold uppercase tracking-wider mb-1 block">Evidence Detail</span>
          ...
          <div className="bg-white px-8 pt-2 border-b border-gray-200 flex gap-8 shrink-0">
             <button 
                ...
                className={`flex items-center gap-2 pb-3 text-[0.8125rem] font-bold border-b-2 transition-colors ${activeTab === 'pdf' ? 'border-[#005EB8] text-[#005EB8]' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
             >
                <FileText className="w-4 h-4" /> PDF View
             </button>
             <button 
                onClick={() => setActiveTab('web')}
                className={`flex items-center gap-2 pb-3 text-[0.8125rem] font-bold border-b-2 transition-colors ${activeTab === 'web' ? 'border-[#005EB8] text-[#005EB8]' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
             >
                <Globe className="w-4 h-4" /> Web URL
             </button>
             <button 
                onClick={() => setActiveTab('image')}
                className={`flex items-center gap-2 pb-3 text-[0.8125rem] font-bold border-b-2 transition-colors ${activeTab === 'image' ? 'border-[#005EB8] text-[#005EB8]' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
             >
          ...
          <div className="flex-1 overflow-y-auto p-8 flex justify-center">
             <div className="bg-white w-full max-w-[50rem] rounded border border-gray-200 shadow-sm p-12 min-h-full">
                
                {/* PDF Content Mockup */}
                <div className="flex justify-between text-[0.625rem] text-gray-300 mb-12 uppercase tracking-widest border-b border-gray-100 pb-2">
                ...
                <p className="text-gray-700 text-[0.9375rem] leading-relaxed mb-6">
                ...
                <p className="text-gray-700 text-[0.9375rem] leading-relaxed mb-10">
                ...
                <div className="relative mb-8">
                  <div className="absolute left-[-3.25rem] top-0 bottom-0 flex items-center">
                     <div className="bg-[#005EB8] text-white p-1.5 rounded-full shadow-md z-10 relative">
                        <FileText className="w-[0.875rem] h-[0.875rem]" />
                     </div>
                     <div className="w-[0.125rem] h-full bg-[#005EB8]/20 absolute left-1/2 -translate-x-1/2"></div>
                  </div>
                  
                  <div className="bg-[#FDE047] border-l-[0.375rem] border-[#005EB8] p-6 shadow-sm">
                     <div className="flex items-center gap-3 mb-3">
                        <span className="text-[0.625rem] font-bold text-[#005EB8] uppercase tracking-wider">AI Suggested Solution</span>
                        <span className="bg-[#005EB8] text-white text-[0.5625rem] px-2 py-0.5 rounded uppercase font-bold tracking-wider">Direct Match</span>
                     </div>
                     <p className="text-gray-900 font-bold text-[0.9375rem] leading-relaxed italic">
                ...
                <p className="text-gray-700 text-[0.9375rem] leading-relaxed">
          ...
          <div className="bg-white border-t border-gray-200 p-5 flex justify-center gap-4 shrink-0">
             <button className="w-[18.75rem] bg-[#005EB8] hover:bg-[#004A99] text-white font-bold py-3 rounded-lg shadow-sm transition-colors text-[0.8125rem]">
                Apply to Treatment Plan
             </button>
             <button 
                onClick={() => setIsInformLeadModalOpen(true)}
                className="w-[18.75rem] bg-white border-2 border-[#005EB8] text-[#005EB8] hover:bg-blue-50 font-bold py-3 rounded-lg transition-colors text-[0.8125rem]"
             >
